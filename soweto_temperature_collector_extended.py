"""
Soweto Temperature Data Collector (Extended)
...
def fetch_soweto_temperature(start_date_str, end_date_str, output_dir="temperature_data"):
    ...
    # Save to CSV
    output_file = os.path.join(output_dir, "soweto_temperature_data_extended.csv")
...
if __name__ == "__main__":
    ...
    if not temp_data.empty:
        ...
        try:
            ...
            original_file = os.path.join(output_dir, "soweto_temperature_data.csv")
            backup_file = os.path.join(output_dir, "soweto_temperature_data_original.csv")
            if os.path.exists(original_file) and not os.path.exists(backup_file):
                shutil.copy2(original_file, backup_file)
                print(f"Original temperature data backed up to {backup_file}")
        except Exception as e:
